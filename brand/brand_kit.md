...
**Dandelion** is the canonical mark, and the project takes its name from it: a DAG (three fork sources converging to a diamond merge node), rotated 35° clockwise from vertical. The rotation gives it a windborne-seed-head reading — three light strands carried sideways into a heavier core — and structurally breaks the upright-trident silhouette that would otherwise rhyme with the USB icon.
