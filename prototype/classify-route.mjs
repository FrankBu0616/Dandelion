// Model-based context-route classifier — browser version.
//
// Mirror of scripts/classify-route.mjs. Calls the model via the browser
// providers module. Returns the same { kind, summary, choices } shape the
// rest of the prototype expects. Falls back to additional_context if the
// model output cannot be parsed — never throws into the UI.
 ...
 * Returns { kind, summary, choices }.
 *
 * @param {Plant[]} graftedPlants
 * @param {{ model?: string, provider?: string, temperature?: number }} opts
 */
export async function classifyRouteWithModel(graftedPlants, opts = {}) {
  ...
  try {
    raw = await chat(
      ...
      { temperature, model: opts.model, provider: opts.provider },
    );
  } catch (err) {
