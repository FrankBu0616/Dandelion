// Continuation-prompt builders — browser version.
//
// Mirrors scripts/server/prompts.mjs. The two flows used by the live app:
//   - buildDynamicContinuationPrompt: post-graft continuation, given the
//     current main conversation and grafted plants.
//   - renderConflict: render a material_conflict route as the JSON shape
//     the UI expects (deliberately does NOT call the model).
//
// buildScenarioContinuationPrompt is also exported for parity with the
// server module — used by the curated scenario flow.
...
/**
 * Build the continuation prompt for the live continue flow.
 * Called after a graft classified as additional_context — caller supplies
